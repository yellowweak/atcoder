...
int main(){
    ...
    grid dp(H+1, vll(W+1, mx));
    rep(i,1,H+1){
        rep(j,1,W+1){
            dp[i][j] = min(g[i-1][j-1], min(dp[i-1][j]+C, dp[i][j-1]+C));
            if(i!=1||j!=1)  ans = min(ans, min(dp[i-1][j], dp[i][j-1])+g[i-1][j-1]+C);
        }
    }
    rep(i,0,H+1) dp[i][0]=mx;
    rep(i,0,W+1) dp[0][i]=mx;
    rep(i,0,H){
        int l=0, r=W-1;
